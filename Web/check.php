...
if (isset($_POST['image'])) {
    ...
    $handle = fopen('./TEMP.png', 'w');
    fwrite($handle, $data);
    
    exec("python Main.py -i TEMP.png > output.txt");
    ...
    /*shell_exec('cd ./ && /usr/bin/python ./Main.py -i ./TEMP.png 1> ./op.txt 2> ./error.txt');
    
    $h = fopen('./op.txt', 'r');
    $result = array();
    while (!feof($h)) {
        array_push($result, fgets($h));
    }
